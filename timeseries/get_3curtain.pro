function get_3curtain,infile1=infile1,infile2=infile2,infile3=infile3,tracenumber=tracenumber,tracername=tracername
...
O3c=fltarr(nlon,nlat,nlev,24)&O3c(*,*,*)=!values.f_NaN
 ...
 CTM_Get_Data, DataInfo3,File=InFile3,'IJ-AVG-$',tracer=tracernumber
...
for n_tau=0,n_elements(Tau0b)-1 do begin
  ...
  Tau0c = DataInfo3[*].Tau0
  Tau0c = Tau0c[ Uniq( Tau0c, Sort( Tau0c ) ) ]
time3=lonarr(24)
for n_tau=0,n_elements(Tau0c)-1 do begin
  t=tau2yymmdd(Tau0c[n_tau],/nformat)&time3[n_tau]=t[1]/10000;time[n_tau]=t[0]*100+t[1]/10000
  ;end of the loop for Tau0
endfor
...
ctm_cleanup
  Ind = Where( DataInfo3[*].Tracername eq tracername )
  for n=0,n_elements(Ind)-1 do begin
    i=Ind[n] 
    if Datainfo3[i].tau0 eq Tau0c[n] then begin
    tmp[*,*,*,n]=(*(DataInfo3[i].data))*1e-9&O3c[*,*,*,n]=tmp[*,*,*,n]
    endif else begin
    print,'problems in tau'&stop
    endelse
endfor
...
curtain3=O3c[39,16,0:iztop,*]*1e9
timearray=[time1,time2,time3]
curtain=[transpose(reform(curtain1)),transpose(reform(curtain2)),transpose(reform(curtain3))]
